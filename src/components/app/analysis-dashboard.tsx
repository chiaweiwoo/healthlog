...
type AnalysisStats = {
  ...
  averageQuotaCalories: number | null;
...
type AnalysisEvidence = {
  topCalorieFoods: EntryItem[];
  alcoholContributors: EntryItem[];
  waterContributors: EntryItem[];
  exerciseContributors: EntryItem[];
  highCalorieLowProteinCandidates: EntryItem[];
};
...
export function AnalysisDashboard({ payload }: { payload: AnalysisReportPayload }) {
  const [showEvidence, setShowEvidence] = useState(false);
  const { stats, evidence, summary, rootCauses, focusAreas, profileGaps, confidence } = payload;
  ...
  const confidenceDetails = getConfidenceStyle(confidence);
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-6 space-y-6">
      ...
      {/* 2. DETERMINISTIC LEVEL 1 STATISTICS */}
      ...
      {/* 3. AI INTERPRETED INSIGHTS */}
      <div className="space-y-4">
        
        {/* SUMMARY & DRIVERS (LEVEL 2/3) */}
        <div className="rounded-xl border border-stone-200 bg-stone-50/60 p-4 shadow-sm space-y-4">
          <SectionHeader
          ...
          <div className="text-sm text-stone-700 leading-relaxed bg-white border border-stone-200/60 rounded-lg p-3 shadow-inner">
            {summary || "No summary provided by Gemini."}
          </div>
        ...
        {/* PROFILE GAPS (LEVEL 4) */}
        {profileGaps && profileGaps.length > 0 && (
          <div className="rounded-xl border border-stone-200 bg-stone-50/60 p-4 shadow-sm space-y-4">
            <SectionHeader
      ...
      {/* 4. ANOMALIES & COLLAPSIBLE EVIDENCE packets */}
